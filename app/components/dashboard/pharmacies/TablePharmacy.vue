...
const { data, pending, refresh } = await useFetch<{
    ...
    total: number
    totalPages: number
}>('/api/pharmacies', { query: queryParams })

const pharmacists = computed(() =>
    (data.value?.pharmacists ?? []).map((u: any) => ({
        id: u.id,
...
async function handleSubmit(payload: any) {
    ...
    try {
        if (modalMode.value === 'add') {
            await $fetch('/api/pharmacies', { method: 'POST', body: payload })
            showSnackbar('Pharmacist created successfully')
        } else if (modalMode.value === 'edit') {
            await $fetch('/api/pharmacies', { method: 'PUT', body: payload })
            showSnackbar('Pharmacist updated successfully')
        } else if (modalMode.value === 'delete') {
            await $fetch('/api/pharmacies', { method: 'DELETE', body: payload })
            showSnackbar('Pharmacist removed successfully')
        }
...
<template>
    <v-card-item class="pb-2 px-0 pt-0">
        <div class="d-flex flex-wrap align-center justify-space-between ga-3">
            <div class="min-w-0">
                <v-card-title class="text-h5 text-md-h3">Pharmacy Management</v-card-title>
                <v-card-subtitle class="mt-1">Manage pharmacy staff accounts</v-card-subtitle>
            </div>
            <v-btn v-if="can('pharmacy.create')" color="primary" variant="flat" size="large" prepend-icon="mdi-plus"
    ...
    <UiTitleCard class-name="px-0 pb-0 rounded-md">
        ...
        <v-table class="bordered-table" hover density="comfortable">
            <thead class="bg-containerBg">
                <tr>
                    <th class="text-no-wrap text-left text-caption font-weight-bold text-uppercase">Pharmacist</th>
            ...
            <tbody>
                ...
                <tr v-else-if="pharmacists.length === 0">
                ...
                <tr v-else v-for="user in pharmacists" :key="user.id">
                    <td class="py-3">
                    ...
                    <td class="py-3 text-right">
                        <v-btn v-if="can('pharmacy.edit')" icon="mdi-pencil-outline" variant="text" size="small"
                            color="secondary" density="comfortable" @click="openEdit(user)" />
                        <v-btn v-if="can('pharmacy.delete')" icon="mdi-delete-outline" variant="text" size="small"
        ...
        <div class="d-flex align-center justify-space-between px-4 py-2">
            <span class="text-caption text-medium-emphasis">
                Showing {{ pharmacists.length }} of {{ totalItems }} pharmacists
            </span>
            <v-pagination
                v-if="totalPages > 1"
                v-model="currentPage"
    ...
    <div class="text-xs-center">
        <v-dialog v-model="dialog" width="480">
            <PharmacyModal :loading="actionLoading" :mode="modalMode" :user="selectedUser" @submit="handleSubmit"
                @cancel="closeModal" />
        </v-dialog>
    </div>
