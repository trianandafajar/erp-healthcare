...
watch(() => props.user, (u) => {
    if (u && props.mode === 'edit') {
        ...
        form.value = { full_name: '', email: '', status: 'active' }
    }
}, { immediate: true })

const config = computed(() => {
    const map = {
        add: { title: 'Add Receptionist', icon: 'mdi-account-plus', confirmColor: 'primary', confirmLabel: 'Add Receptionist' },
        edit: { title: 'Edit Receptionist', icon: 'mdi-account-edit', confirmColor: 'primary', confirmLabel: 'Save Changes' },
        delete: { title: 'Remove Receptionist', icon: 'mdi-account-remove', confirmColor: 'error', confirmLabel: 'Remove' }
    }
        ...
        <template v-if="mode === 'delete'">
            <v-card-text class="pa-5">
                <div class="d-flex flex-column align-center text-center ga-3">
                    ...
                    <div>
                        <p class="text-body-1 font-weight-medium">Are you sure?</p>
                        <p class="text-body-2 text-medium-emphasis mt-1">
                            <strong>{{ user?.full_name }}</strong> ({{ user?.email }}) will be permanently removed.
                        </p>
                    </div>
                </div>
            </v-card-text>
