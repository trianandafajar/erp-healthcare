...
const { data, pending, refresh } = await useFetch(`/api/departments/${entityId}`, {
  key: `superadmin-department-${entityId}`,
})

const { data: apptData, pending: apptPending, refresh: refreshAppt } = await useFetch(`/api/departments/${entityId}/appointments`, {
  key: `superadmin-department-${entityId}-appointments`,
  query: { limit: 50 },
})
      ...
      <v-card-item class="pa-5">
        <div class="d-flex align-center justify-space-between flex-wrap ga-4">
          <div class="d-flex align-center ga-4">
            <v-btn icon="mdi-arrow-left" variant="text" @click="navigateTo({ query: { section: 'departments' } })" />
          ...
          <v-btn variant="tonal" color="primary" prepend-icon="mdi-pencil"
            @click="navigateTo({ query: { section: 'department-edit', entityId } })">
            Edit
          </v-btn>
        </div>
      </v-card-item>
    ...
    <v-card variant="flat">
      <v-tabs v-model="activeTab" color="primary" density="comfortable">
        ...
        <v-tab value="doctors" prepend-icon="mdi-doctor">
          Doctors
          <v-chip class="ml-2" size="x-small" color="primary" variant="tonal">{{ stats?.total_doctors ?? 0 }}</v-chip>
        </v-tab>
      ...
      <v-window v-model="activeTab">
        ...
        <v-window-item value="info">
          <v-card-text class="pa-6">
            <v-row>
              <v-col cols="12" md="6">
                <v-card variant="outlined" rounded="md" class="h-100">
                  <v-card-item>
                    <template #prepend>
                    ...
                    <v-card-title class="font-weight-bold">Department Details</v-card-title>
                  </v-card-item>
                  <v-divider />
                  <v-card-text>
                    <v-row dense>
                      ...
                      <v-col cols="6">
                        ...
                        <v-chip size="small" color="primary" variant="tonal">{{ department.code || '-' }}</v-chip>
                      </v-col>
                      <v-col cols="6">
                        <div class="text-caption text-medium-emphasis">Doctors</div>
                        <div class="text-body-1 font-weight-medium">{{ stats?.total_doctors ?? 0 }}</div>
                      </v-col>
                      <v-col cols="6">
                      ...
                      <v-col cols="6" class="mt-5">
                        ...
                        <div class="text-body-2">{{ formatDate(department.created_at) }}</div>
                      </v-col>
                      <v-col cols="6" class="mt-5">
                        <div class="text-caption text-medium-emphasis">Last Updated</div>
                        <div class="text-body-2">{{ formatDate(department.updated_at) }}</div>
                      </v-col>
                    </v-row>
                  </v-card-text>
                </v-card>
              ...
              <v-col cols="12" md="6">
                <v-card variant="outlined" rounded="md" class="h-100">
                  ...
                  <v-divider />
                  <v-card-text>
                    <div class="text-body-2" style="min-height:140px; line-height:1.7;">
                ...
                <td class="py-3">
                  <div class="d-flex align-center ga-3">
                    <v-avatar size="32" color="primary" variant="tonal">
                      <v-img v-if="doc.profiles?.avatar_url" :src="doc.profiles.avatar_url" cover />
                      <span v-else class="text-caption font-weight-bold">{{ getInitials(doc.profiles?.full_name) }}</span>
                    </v-avatar>
                    <div>
                      <div class="text-body-2 font-weight-medium">{{ doc.profiles?.full_name ?? '-' }}</div>
                      <div class="text-caption text-medium-emphasis">{{ doc.profiles?.email ?? '-' }}</div>
                    </div>
                  </div>
                </td>
                <td class="py-3 text-body-2">{{ doc.specialization ?? '-' }}</td>
                <td class="py-3 text-body-2 text-medium-emphasis">{{ doc.phone ?? '-' }}</td>
                <td class="py-3 text-body-2">{{ doc.experience_years ? `${doc.experience_years} yrs` : '-' }}</td>
                <td class="py-3 text-body-2">{{ formatCurrency(doc.consultation_fee) }}</td>
                <td class="py-3">
                  <v-chip :color="doc.is_available ? 'success' : 'error'" size="small" variant="tonal">
            ...
            <tbody>
              ...
              <tr v-for="appt in appointments" :key="appt.id">
                ...
                <td class="py-3 text-body-2">
                  ...
                  <div v-if="appt.doctors?.specialization" class="text-caption text-medium-emphasis">{{ appt.doctors.specialization }}</div>
                </td>
                <td class="py-3">
