...
const { data, pending } = await useFetch(`/api/nurses/${entityId}`, {
  key: `superadmin-nurse-${entityId}`,
})

const { data: vitalData, pending: vitalPending } = await useFetch(`/api/nurses/${entityId}/vital-signs`, {
  key: `superadmin-nurse-${entityId}-vitals`,
  query: { limit: 20 },
})

const { data: careData, pending: carePending } = await useFetch(`/api/nurses/${entityId}/care-notes`, {
  key: `superadmin-nurse-${entityId}-care-notes`,
  query: { limit: 20 },
})
      ...
      <v-card-item class="pa-5">
        <div class="d-flex align-center justify-space-between flex-wrap ga-4">
          <div class="d-flex align-center ga-4">
            <v-btn icon="mdi-arrow-left" variant="text" @click="navigateTo({ query: { section: 'nurses' } })" />
            ...
            <div>
              ...
              <div class="d-flex align-center ga-2 mt-1 flex-wrap">
                <v-chip v-if="department" size="small" color="secondary" variant="tonal" label>
                  {{ department.name }}
                  <span v-if="department.code" class="ml-1 text-medium-emphasis">({{ department.code }})</span>
                </v-chip>
    ...
    <v-card variant="flat">
      <v-tabs v-model="activeTab" color="primary" density="comfortable">
        ...
        <v-tab value="vitals" prepend-icon="mdi-heart-pulse">
          Vital Signs
          <v-chip class="ml-2" size="x-small" color="primary" variant="tonal">{{ stats?.total_vital_signs ?? 0 }}</v-chip>
        </v-tab>
        <v-tab value="care-notes" prepend-icon="mdi-note-text">
          Care Notes
          <v-chip class="ml-2" size="x-small" color="primary" variant="tonal">{{ stats?.total_care_notes ?? 0 }}</v-chip>
        </v-tab>
      </v-tabs>
              ...
              <v-col cols="12" md="6">
                <v-card variant="outlined" rounded="lg" class="h-100">
                  <v-card-item class="py-4">
                    ...
                    <div>
                      <div class="text-h6 font-weight-bold">Nurse Details</div>
                      <div class="text-caption text-medium-emphasis">Professional profile and department information</div>
                    </div>
                  </v-card-item>
                  <v-divider />
                  <v-card-text class="pa-6">
                    <v-row>
                      ...
                      <v-col cols="6">
                        ...
                        <div class="text-body-2">{{ nurse.phone || '-' }}</div>
                      </v-col>
                      <v-col cols="6">
                        <div class="text-overline text-medium-emphasis">Experience</div>
                        <div class="text-body-2">{{ nurse.experience_years || 0 }} Years</div>
                      </v-col>
                      <v-col cols="12">
                        <div class="text-overline text-medium-emphasis">Department</div>
                        <div class="text-body-1 font-weight-medium">{{ department?.name || '-' }}</div>
                      </v-col>
                      <v-col cols="6">
                        <div class="text-overline text-medium-emphasis">Created</div>
                        <div class="text-body-2">{{ formatDate(nurse.created_at) }}</div>
                      </v-col>
                      <v-col cols="6">
                        <div class="text-overline text-medium-emphasis">Last Updated</div>
                        <div class="text-body-2">{{ formatDate(nurse.updated_at) }}</div>
                      </v-col>
                    </v-row>
                  </v-card-text>
                </v-card>
                ...
                <v-card variant="outlined" rounded="lg" class="h-100">
                  <v-card-item class="py-4">
                    ...
                    <div>
                      ...
                      <div class="text-caption text-medium-emphasis">Account status and availability</div>
                    </div>
                  </v-card-item>
                  <v-divider />
                  <v-card-text class="pa-6">
                    <div class="d-flex align-center ga-4 mb-6">
                      <v-avatar size="72" color="primary" variant="tonal">
                        <v-img v-if="profile?.avatar_url" :src="profile.avatar_url" cover />
                        <span v-else class="text-h5 font-weight-bold">{{ getInitials(profile?.full_name) }}</span>
                      </v-avatar>
                      <div>
                        <div class="text-h6 font-weight-bold">{{ profile?.full_name || '-' }}</div>
                        <div class="text-body-2 text-medium-emphasis">{{ profile?.email || '-' }}</div>
                        <v-chip class="mt-3" :color="profile?.status === 'active' ? 'success' : 'warning'" variant="tonal" size="small">
                          {{ profile?.status || '-' }}
                        </v-chip>
                      </div>
                    </div>
                    <v-divider class="mb-6" />
                    <div class="text-overline text-medium-emphasis mb-2">Availability</div>
                    <v-chip :color="nurse.is_available ? 'success' : 'error'" variant="tonal" size="default" class="px-4">
                      <v-icon start :icon="nurse.is_available ? 'mdi-check-circle' : 'mdi-close-circle'" />
            ...
            <tbody>
              ...
              <tr v-for="v in vitals" :key="v.id">
                ...
                <td class="py-3 text-body-2">
                  ...
                  <div v-if="v.patients?.medical_record_number" class="text-caption text-medium-emphasis">{{ v.patients.medical_record_number }}</div>
                </td>
            ...
            <tbody>
              ...
              <tr v-for="cn in careNotes" :key="cn.id">
                ...
                <td class="py-3 text-body-2">
                  ...
                  <div v-if="cn.patients?.medical_record_number" class="text-caption text-medium-emphasis">{{ cn.patients.medical_record_number }}</div>
                </td>
                <td class="py-3">
                  <v-chip :color="careNoteColor[cn.category] ?? 'default'" size="small" variant="tonal" label>
                    {{ cn.category ?? '-' }}
                  </v-chip>
                </td>
                <td class="py-3 text-body-2 text-medium-emphasis" style="max-width: 280px; white-space: normal;">
                  {{ cn.note ?? '-' }}
                </td>
