...
<template>
    <div class="d-flex justify-space-between align-center mb-6 flex-wrap ga-3">
        ...
        <v-btn color="secondary" variant="tonal" prepend-icon="mdi-printer-outline" to="/receptionist/queue/print">Print
            Number</v-btn>
    </div>
        ...
        <div class="px-4 py-3">
            <v-row dense>
                ...
                <v-col cols="6" sm="3" md="1">
                    <v-text-field v-model="checkInFrom" type="time" label="From" variant="outlined" density="compact"
                        hide-details />
                </v-col>
                <v-col cols="6" sm="3" md="1">
                    <v-text-field v-model="checkInTo" type="time" label="To" variant="outlined" density="compact"
                        hide-details />
                </v-col>
            </v-row>
                    ...
                    <td class="text-right">
                        <div class="d-flex justify-end ga-2">
                            <v-btn size="small" color="secondary" variant="tonal"
                                @click="openDetail(item.id)">Detail</v-btn>
                            <v-btn v-if="nextQueueAction(item.status) && can('queue.update')" size="small"
                            ...
                            <v-btn v-else size="small" color="secondary" variant="text" disabled>
                                Closed
                            </v-btn>
                        </div>
                    </td>
    ...
    <v-dialog v-model="detailDialog" max-width="560">
        <v-card v-if="selectedQueue">
            ...
            <v-card-actions class="justify-end">
                ...
                <v-btn v-if="nextQueueAction(selectedQueue.status)"
