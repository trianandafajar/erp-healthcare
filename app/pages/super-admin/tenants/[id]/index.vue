...
import ReportPage from '~/components/pages/admin/ReportPage.vue'
import TableDoctors from '~/components/dashboard/doctors/TableDoctors.vue'
import TableNurse from '~/components/dashboard/nurses/TableNurse.vue';
import TablePatient from '~/components/dashboard/patient/TablePatient.vue';
import TableUser from '~/components/dashboard/users/tableUser.vue';
import TableLog from '~/components/dashboard/LogActivity/TableLog.vue';
import SettingsPage from '~/components/pages/admin/SettingsPage.vue';
import TableDepartments from '~/components/dashboard/departments/TableDepartments.vue'
...
<template>
    <TenantDetailSidebar v-model="sidebarOpen" :slug="tenantSlug" :active-section="section"
    ...
    <template v-else-if="section === 'doctors'">
        <TableDoctors />
    </template>

    <template v-else-if="section === 'nurses'">
        <TableNurse />
    </template>

    <template v-else-if="section === 'patients'">
        <TablePatient />
    </template>
    ...
    <template v-else-if="section === 'log-activity'">
        <TableLog :actor-id="actorId" />
    </template>
    ...
    <template v-else>
        ...
        <template v-else>
            ...
            <v-card elevation="0" class="rounded-md mb-4">
                <v-row no-gutters>
                    <v-col v-for="(stat, i) in statCards" :key="stat.label" cols="6" sm="4" md="2"
                        class="pa-4 text-center" :class="i < statCards.length - 1 ? 'border-e' : ''">
                        ...
                        <v-row>
                            ...
                            <v-col cols="12" md="6">
                                <v-card elevation="0" variant="outlined" :style="{ borderColor: '#e0e0e0' }"
                                    ...
                                    <v-card-item>
                                        <v-card-title
                                            class="text-subtitle-1 font-weight-bold">Subscription</v-card-title>
                                    </v-card-item>
                                    <v-divider />
                                    <v-card-text>
                                ...
                                <v-card elevation="0" variant="outlined" :style="{ borderColor: '#e0e0e0' }"
                                    ...
                                    <v-card-text>
                                        <div v-for="(val, key) in tenant?.appointment_breakdown" :key="key"
                                            ...
                                            <v-chip :color="statusColors[key] ?? 'secondary'" variant="tonal"
                                    ...
                                    <v-table density="comfortable">
                                        <thead class="bg-containerBg">
                                            <tr>
                                                ...
                                                <th class="text-left text-caption font-weight-bold text-uppercase">
                                                    Doctor</th>
                                                <th class="text-left text-caption font-weight-bold text-uppercase">Date
                                                </th>
                                                <th class="text-left text-caption font-weight-bold text-uppercase">
                                                    Status</th>
                                            </tr>
                        ...
                        <v-row>
                                ...
                                <v-card elevation="0" variant="outlined" :style="{ borderColor: '#e0e0e0' }"
                                    ...
                                    <v-card-text>
                                        ...
                                        <div class="d-flex ga-3 align-stretch">
                                            <v-avatar size="72" rounded="lg" color="grey-lighten-3"
                                                class="flex-shrink-0">
                                            ...
                                            <div class="photo-dropzone flex-grow-1 d-flex flex-column align-center justify-center ga-1 rounded-lg"
                                                ...
                                                @dragover="onLogoDragOver" @dragleave="onLogoDragLeave"
                                                @drop="onLogoDrop" @click="logoInputRef?.click()">
                                                <v-icon
                                        ...
                                        <div v-if="logoError"
                                            class="text-caption text-error mt-2 d-flex align-center ga-1">
                            ...
                            <v-col cols="12" md="5">
                                <v-card elevation="0" variant="outlined" :style="{ borderColor: '#e0e0e0' }"
                                    class="rounded-md h-100">
                                    <v-card-item>
                                        <v-card-title class="text-subtitle-1 font-weight-bold">Brand
                                            Color</v-card-title>
                                        <v-card-subtitle>Primary accent color used across the tenant's
                                            app</v-card-subtitle>
                                    </v-card-item>
                                    <v-divider />
                                    <v-card-text>
                        ...
                        <v-table hover density="comfortable">
                            ...
                            <tbody>
                                ...
                                <tr v-else v-for="user in users" :key="user.id">
                                    ...
                                    <td class="py-2 text-body-2 text-medium-emphasis">{{ formatDate(user.created_at) }}
                                    </td>
                                </tr>
                            </tbody>
                        </v-table>
                        ...
                        <v-table density="comfortable">
                            ...
                            <tbody>
                                <tr v-if="!tenant?.recent_activity?.length">
                                    <td colspan="4" class="text-center py-6 text-medium-emphasis">No activity recorded
                                    </td>
                                </tr>
                                <tr v-else v-for="log in tenant.recent_activity" :key="log.id">
                                    ...
                                    <td class="py-2 text-body-2 text-medium-emphasis">{{ formatDate(log.created_at) }}
                                    </td>
                                </tr>
                            </tbody>
                        </v-table>
